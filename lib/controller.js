...
class Controller extends EventEmitter {
  ...
  *startOne(fun, c, pos) { //start function with object and supply event emit after end
    ...
    const helper=(fun)=>{
      ...
      return fun(c)
            .then((res) => this.emit("arbeiten", null, res, temp,c)) // emit result
              .catch((err) => this.emit("arbeiten", err, null, temp,c)) // emit error
  ;
  ...
  waiterEnd(err, obj, pos,input) {
    ...
    if (err) {
      ...
      //store error
        ...
        if ( this.errHandler(pos, err,input) ){  //too many errors
          this.done=true;
        }
    }
    if (pos !== undefined) {
      ...
      if (obj) {
        this.objHandler(pos, obj,input); //user handling of result
      }
  ...
  *
  ...
  checkRunning(checkEntry ){
      this.parallel = this.parallelsave;
    for (const entry of this.open.entries()) {
      if ( checkEntry( entry, Object.assign( entry[0],{ diff: process.hrtime(entry[0].start)} )) ){
    break;
      }
  ...
  let slow;
    ...
    return {
      ...
      slowtime: process.hrtime(slow.start),
      slowest: slow,

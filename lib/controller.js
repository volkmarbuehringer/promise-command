...
class Controller extends EventEmitter {
  ...
  *
  ...
  const r=a?a.diff[0]*1e9+a.diff[1]:0;
          ...
          next: function() {
              for (;;) {
                  if (nextIndex < array.length) {
                    const merk=nextIndex;
                      const value = array[nextIndex++];
//                      console.error("vordel",value,nextIndex,array.length,array[nextIndex]);
//                        console.error("nachdel",value,nextIndex,array.length,array[nextIndex]);
                      if (value && "input" in value && testFun(value)) {
                        delete array[merk];
      ...
      if (this.open.size >= this.parallel) {
          la = yield Promise.resolve();
              ...
              if (next.done) {
                  ...
                  la = yield* this.startOne(this.fun, next.value.input||next.value);
              }
          }
          if (la) {
  ...
  *
  waiterFinished(timer) { //block with a sleep function, if starttime is not yet reached
      const r=[];
      let flag=false;
 setTimeout(()=>flag=true, timer).unref();
 this.setEndFlag();
          while ( !flag && this.open.size>0){
          r.push( yield Promise.resolve());
        }

...
class Controller extends EventEmitter {
  ...
  *
  startAll(iterator) { //start  function with an iterator n times
    ...
    for (const c of iterator) {
        r.push(yield* this.startOne(this.fun, c));
    }
    return r;
  ...
  *
  startAllNext(iterator) { //start  function with an iterator n times
    this.sleepval = null;
    const r=[];
    for (const c of iterator) {
      if ( c && c.input ){
          r.push(yield* this.startOne(this.fun, c.input));
      }
  ...
  *
  ...
  return yield* this.startAll(res);

...
class Controller extends EventEmitter {
  ...
  *
  ...
  waiterEnd(err, obj, pos,input) {
    ...
    if (pos !== undefined) {
      ...
      if (obj) {
        this.objHandler(pos, obj,input); //store endresult in order of start like Promise.all
      }
  ...
  *
  ...
  checkRunning(checkEntry, maxcount = 1 ){
    let entry = [null, null];
    let p = 0;
    for (entry of this.open.entries()) {
      if ( checkEntry( entry, Object.assign( entry[0],{ diff: process.hrtime(entry[0].start)} ), p++ ) ){
        this.open.delete(entry[0]);
      }
      if ( p > maxcount ){
        break;
      }
